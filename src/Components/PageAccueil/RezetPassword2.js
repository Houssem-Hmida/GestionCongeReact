...
class RezetPassword2 extends Component {
    constructor(props) {
      ...
      doRezetPassword = async (event) => {
        ...
        AuthenticationService.compare(this.state.email,this.state.password,this.state.confirmepassword).then(
          (response)=> {
              if(response.result==1){
                             
                AuthenticationService.rezetpassword(this.state.email,this.state.password,this.state.confirmepassword).then(
                          (resp)=>{
                            if(resp.result==1){
